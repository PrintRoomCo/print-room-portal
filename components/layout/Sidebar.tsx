...
import { getNavigationItems, type NavIconKey } from '@/lib/nav/portal-nav'
...
export function Sidebar({ children, customer }: SidebarProps) {
  ...
  // Every nav item renders as a row of the hand-drawn inline SVG menu so
  // they all share identical geometry (row height, icon size, text size).
  // Permission gating flows through `navigation` (already filtered by
  // getNavigationItems); display order comes from PORTAL_NAV_ITEMS.
  const visibleRows = navigation.map((item) => ({
    id: item.iconKey,
    label: item.name,
    href: item.href,
    isActive:
...
// Icon artwork for each nav row, drawn in a 24×24 local space and scaled
// up by the SVG menu. Strokes come from .cls-line so weights stay uniform.
// Keyed by every NavIconKey so a new nav item can't ship without an icon.
const ROW_ICONS: Record<NavIconKey, React.ReactElement> = {
  catalogue: (
    <>
      {/* Tall paper bag with top fold — t.e. shop. */}
      <rect className="cls-line" x={8} y={2} width={8} height={20} rx={0.5} />
      <line className="cls-line" x1={8} y1={5.5} x2={16} y2={5.5} />
    </>
  ),
  tracking: (
    <>
  ...
  orders: (
    <>
      ...
      />
      <circle className="cls-line" cx={12} cy={12} r={3} />
    </>
  ),
  proofs: (
    <>
  ...
  inventory: (
    /* Stacked stock boxes — isometric cube. */
    <path
      className="cls-line"
