...
- Modify: `app/(portal)/orders/[id]/proof/edit/page.tsx` — `ALLOWED_ROLES` set (`:54-57`).
- Modify: `app/(portal)/orders/[id]/proof/page.tsx` — `ALLOWED_EDIT_ROLES` set (`:9`).
- Modify: `app/api/proofs/[id]/amendment-requests/route.ts` — `ALLOWED_ROLES` set (`:70`).
...
## Task 4: Portal — proof-amendment role gates (3 route/page sets)

These three sets hardcode `new Set(['org_admin','buyer'])`. They gate proof-amendment access — a miss silently breaks it for staff members (spec Acceptance).
...
Create `app/api/proofs/__tests__/amendment-role-allowlist.test.ts`. Rather than booting the route, assert the allow-list constant's contents. First export it:

In `app/api/proofs/[id]/amendment-requests/route.ts`, change the existing `const ALLOWED_ROLES = new Set(['org_admin', 'buyer'])` to be exported (keep its location at `:70`):
...
```ts
import { describe, it, expect } from 'vitest'
import { ALLOWED_ROLES } from '../[id]/amendment-requests/route'
...
> Note: the test file path uses a relative import that resolves the dynamic `[id]` segment — if Vitest's resolver objects to the bracketed path, move the test to `app/api/proofs/[id]/amendment-requests/__tests__/role-allowlist.test.ts` and import `'../route'`.

- [ ] **Step 2: Run it — verify it fails**

Run: `npx vitest run app/api/proofs/[id]/amendment-requests/__tests__/role-allowlist.test.ts`
Expected: FAIL on `expect(ALLOWED_ROLES.has('staff')).toBe(true)` **before** Step 3's edit — or import error if `ALLOWED_ROLES` not yet exported. (Author Step 1's export change, then run; it fails on the `'staff'` assertion because the set still contains `'buyer'`.)

- [ ] **Step 3: Flip all three allow-lists**

`app/(portal)/orders/[id]/proof/edit/page.tsx` (the `ALLOWED_ROLES` set at `:54-57`):

```ts
const ALLOWED_ROLES = new Set(['org_admin', 'staff'])
```

`app/(portal)/orders/[id]/proof/page.tsx` line 9 (`ALLOWED_EDIT_ROLES`):

```ts
const ALLOWED_EDIT_ROLES = new Set(['org_admin', 'staff'])
```

(`amendment-requests/route.ts` was already flipped in Step 1.)
...
```bash
git add "app/(portal)/orders/[id]/proof/edit/page.tsx" "app/(portal)/orders/[id]/proof/page.tsx" "app/api/proofs/[id]/amendment-requests/route.ts" "app/api/proofs/[id]/amendment-requests/__tests__/role-allowlist.test.ts"
git commit -m "refactor(portal): role value buyer->staff in proof-amendment gates"
```
...
const sql = readFileSync(
  join(__dirname, '../../../supabase/migrations/20260602090000_rename_role_buyer_to_staff.sql'),
  'utf8',
)
...
In `role/route.ts` line 15, add `export`:
...
`src/components/b2b-accounts/MembersPanel.tsx`:
...
A guard test that fails red while any role-value literal survives in live code and goes green only when the rename is complete. This is the spec's done-condition, automated.
...
Expected: PASS. If it lists hits, replace each role-value literal and re-run.

- [ ] **Step 3: Write + run the gate test (staff)**

Create `print-room-staff-portal/src/lib/__tests__/no-buyer-literal.test.ts` with the **same body**, but extend the exclude list to also drop the Shopify port scripts and the synced vendor copy — add these two excludes to the `git grep` invocation:

```
":(exclude)scripts/shopify-orders-port/**" ":(exclude)vendor/**"
```
...
**2. Placeholder scan:** No "TBD"/"similar to"/"add validation". Every step shows the exact literal change with surrounding code. The one spot that says "whatever the grep surfaces" (Task 5) is deliberate — the spec explicitly mandates a grep sweep rather than hand-enumeration — and it is bounded by Step 3's `tsc` + Task 8's gate, so completeness is machine-verified, not left to judgement.

**3. Type consistency:** `role` union is `'org_admin' | 'staff'` in all three declaration sites (`types/company.ts`, `lib/company.ts` AccessInput, `lib/checkout/server.ts`). The internal flag `isBuyer` keeps its name everywhere (Task 2 fixes only its derivation); no call site renamed it, so no signature drift. `BuyerScopeError` class name unchanged across `submit.ts` + its test (Task 3) + the API catch in `app/api/checkout/route.ts` (untouched — it catches the class, not a literal). `MemberRole` union (`'org_admin' | 'staff'`) is consistent between `EditRoleDialog.tsx` and the route's `ALLOWED_ROLES`/`isMemberRole`.

**Anchor drift adapted:** (a) The data `UPDATE` affects **0 rows today** (verified) — authored regardless, with a re-count gate. (b) `lib/company.ts` derivation spans `:208-209` *and* `:217` (the `isBuyer` field) — Step 3 touches `:209` only; `:217` is a field assignment of the already-correct `isBuyer`, no change needed. (c) Excluded `vendor/print-room-onboarding/**` (synced package, separate `'staff'` audience) and `scripts/shopify-orders-port/**` (`buyerExperienceConfiguration`) from the gate — flagged, not renamed.
