...
> ⚠️ **Before executing, check overlap:** a branch `feat/product-fulfilment-type` already exists in this repo (and on origin). Confirm it does not already add a catalogue-item fulfilment override (it may be master-product-only work). If it overlaps, reconcile before starting rather than duplicating.
...
Extract the patch builder. Introduce this exported function above `PATCH`:
...
Create `src/components/catalogues/__tests__/CatalogueItemEditor.fulfilment.test.tsx`:
...
Add the control to the "Catalogue-scoped details" `Card`. Insert a new `Field` inside the `grid` (after the "Sort order" `Field` at line 356, before "Description"):
...
5. Verify the master is untouched: `SELECT fulfilment_type FROM products WHERE id = '<source_product_id>';` → unchanged.
...
- *Value drives which pill the product appears under (Item 2)* → out of scope here; Plan C consumes the column.

**2. Placeholder scan:** No "TBD"/"similar to". Every code step is complete. The one lookup-by-grep (Task 3 Step 1, locating the loader) is unavoidable — the loader path wasn't pinned during grounding — but it is bounded: the grep key (`lead_time_days_override`) appears only in the loader's select and the editor, and Task 3 Step 4 (`tsc`) verifies the thread-through compiled.

**3. Type consistency:** `FulfilmentType` and `FULFILMENT_TYPES`/`FULFILMENT_TYPE_LABELS` are imported from one source (`@/types/products`) everywhere. `coerceFulfilmentOverride` returns `FulfilmentType | null | undefined`, matching the column. `FormState.fulfilment_type_override` is `string` (`''` = inherit), normalised to `null | FulfilmentType` only at the `buildPatch`/route boundary.

**Anchor drift adapted:** The recon agent claimed a column needed adding — **corrected**: the column already exists (verified via Supabase MCP), so this plan has **no migration**. The editor's server loader path was not pinned during grounding, so Task 3 Step 1 locates it by a grep on a column it provably already selects.
