...
- **2026-08-13 — Phase 0 customer plumbing — done (see below).**

### Phase 1 — Staff authoring + backfill

- **2026-08-13 — 1a route + resolver — done.** (staff `10f50e5e`) `GET/PUT /api/orgs/[orgId]/decorations/[decorationId]/tiers`. New pure `resolve-decoration-tier-writes.ts` modelled on `resolve-base-cost-tier-writes.ts` — which, contrary to the plan's read, **already carries** the structural rules; it is `resolve-pricing-tier-writes.ts` that disclaims them. Rules: sorted, non-overlapping (touching bands count as overlapping), `min >= 1`, at most one open tail and only on the highest band; gaps stay legal because the DB clamps. `decorationLadderRefusal()` refuses `custom`-method and artwork-less decorations. Writes via the Phase-0 RPC with the prior-rows SELECT for the diff; `recordAuditEvent` on every PUT under a new `AUDIT_ACTIONS.DECORATION_PRICING_TIER_REPLACE`. 12 tests.
- **2026-08-13 — 1a UI — done.** (staff `4090c691`) `DecorationLadderEditor` embedded in `DecorationFormDialog`, owning its own load/save (a ladder needs a persisted decoration, so it is independent of the form's Save) and mirroring the route's refusals up-front. `LADDER_INPUT_CLASS` lifted out of `PricingSection` into `src/lib/ui/ladder-input.ts` so both ladders share one definition. `PricingSection` renders the Decoration **and Total** columns read-only when the catalogue has pooling on — Total too, because `TotalInput` writes back into `decoration_unit_price`; garment price stays editable. `CataloguePoolingCard` on the Settings tab renders the (a)–(d) checklist live: **(a) missing ladders blocks activation**, (b)/(c)/(d) are warnings; turning pooling OFF is always allowed. `PATCH /api/catalogues/[id]` accepts + audits `decoration_pooling_enabled`. oem-rules pre-flight satisfied (primitives, `bg-black/[0.06]`, `border-black/10`, `rounded-2xl`/`rounded-full`, `ease-oem`, focus rings). Tests: `PricingSection.pooling.test.tsx` (4), `pooling-readiness.test.ts` (10). *Deviation:* the initial effects tripped `react-hooks/set-state-in-effect`; both components were restructured to the repo's `cancelled`-flag idiom, and the ladder editor swapped `loading` for a `loaded` flag so the guarded (no decoration / refused) branches never need unwinding.
- **2026-08-13 — 1b backfill proposal — done.** (staff `357ba35a`) `scripts/propose-decoration-ladders.ts` + pure `decoration-ladder-backfill.ts` (8 tests). **No `--apply` exists at all.** Proposes on a single donor or agreeing donors; refuses to guess otherwise; treats differing band *edges* as disagreement even at equal prices; drops all-zero donors so an all-in catalogue can never yield a `$0` ladder that would then beat engine pricing everywhere it is attached. Emits CSV per org (stdout or `--out=`).

Staff suite after Phase 1: **3 files / 8 tests failing — exactly the recorded baseline** — 2355 passed (baseline 2321, +34). `tsc` and `eslint` clean against baseline.

### Phase 2 — Pricing engine (the lockstep release)

- **2026-08-13 — 2a shared module — done.** (customer `200d6f0`) `lib/pricing/decoration-pooling.ts`: `pooledQtyByDecoration`, `garmentBandQty` (max rule), `pooledDecorationQty`, `poolSizesForLine`. 21 tests carry spec worked examples A & B verbatim including the anti-transitivity assertion, and pin every §5 eligibility rule in both directions.
- **2026-08-13 — FLAG-OFF PARITY ORACLE — done, before the pooled branches.** `lib/cart/flag-off-parity.test.ts` holds a verbatim copy of the pre-pooling `recomputeProductTierPrices` and recomputes the expected answer every run (an oracle, not a snapshot) over a fixture covering same-product pooling, cross-product isolation, stocked+made_to_order sharing a product, manual_final, bracket-less legacy lines, band edges, referential identity, every band-edge qty edit and every single-line removal. Verified green against the pre-pooling implementation, then kept green through 2b/2c.
- **2026-08-13 — 2b cart — done.** (customer `dccf14a`) Pooled recompute; pooled manual_final lines drop the combined figure for the Σ per-placement sum. New `lib/pricing/decoration-ladder.ts` normalises an authored ladder so the cart's exact-band matching reproduces the DB's clamp at every quantity — proven against a direct implementation of the SQL's three steps across 8 ladder shapes — which is what lets laddered decorations snapshot **exact ladder bands instead of probe output**. `loadCatalogueItemDecorations` reads ladders (one batched select) and seeds PDP first paint from the ladder. Pooled cases added to **both** duplicate cart suites. *Deviation:* `decorationPriceAt` on the PDP now prefers an authored ladder over the probe cache — same source the server RPC reads, exact at every qty, and it removes probe timing as a drift cause.
- **2026-08-13 — 2c checkout server — done.** (customer `16bc917`) Flag rides the existing `b2b_catalogue_items` select (hoisted above the garment price loop, since the flag decides that loop's band qty; widened to `poolLines`); asserted that turning the flag on adds **zero** catalogue-items reads. Poolability resolved **server-side** from `org_decorations` and only when a catalogue opts in, so flag-off adds no query at all. Garment groups gained `bandQty` beside an untouched `totalQty`, with the band qty in the group key so every line in a group provably shares one band; `period_unit_price` keeps `totalQty`. 13 new tests including "never inflate real quantities" (submitted line quantities identical in both flag states). *Plan correction:* the review changelog said no `stockUnitPriceByItem` symbol exists — it does (`submit.ts:841`); the substantive claim it supported (stocked lines are not skipped from today's aggregation) is correct and was honoured. *Finding:* an existing nature coercion (`submit.ts:749-761`) demotes an unsupported `stocked` claim to `made_to_order` before pricing; pooling reads `fulfilment_type` after it, which is the correct order, now pinned by a test.
- **2026-08-13 — 2d amendment RPC — done.** (staff `6998fcf1` + `c77686cd`) `supabase/migrations/20260813130000_pooled_amendment_pricing.sql`, a mechanical transformation of the **0807** body (diffed 0805 vs 0807 first — their only difference is the Xero-effect branch, carried forward verbatim). A test asserts every line differing from 0807 is one of the pooling changes. New `pooled_decoration_qty` helper; per-decoration pooled qty; max rule via `greatest(...)` at `effective_unit_price_for_item` only; flag-gated precedence fix; per-line `repriced`, wired into the amendment review as "This change re-bands N other lines". *Documented limitation:* an amendment's intended state carries no per-line stocked choice, so the pool excludes stock-ONLY items rather than stock-drawing lines.
- **2026-08-13 — 2e staff quote-line — done.** (staff `c77686cd`) Pools derived server-side from `order_id`; **no client payload change**. Pure `maxRuleBandQty` + a loader that runs exactly one query and returns `poolingEnabled: false` when the catalogue has not opted in. Two v1 edges documented in code, both erring toward the HIGHER price: only saved siblings count, and saved lines of the requested item are replaced rather than merged (no line id in the payload).

### Phase 3 — Customer display

- **2026-08-14 — done.** (customer `1b93845`) `decorations[].pooledQty` written by the recompute; pure `lib/pricing/same-artwork-savings.ts` (15 tests) for the pill, tooltip and next-band nudge — pill fires only when the pool exceeds this line, names the largest pool, and the nudge measures to the nearest cheaper break across both ladders, skipping same-price boundaries. `SameArtworkSavings` renders in the cart and the checkout review; the pill is a real button so the explanation is reachable by keyboard and screen reader. PDP static note under Volume Pricing. Copy asserted against spec §8 verbatim, including that no `$` figure ever appears.
