...
- **[prepaid-tag-customer-display-xero-pickfee]** ✅ IMPLEMENTED DEFAULT (pick-fee scope): the fee applies to each stock-on-hand order/partition, not purchase-order-only or prepaid-only orders. This matches spec (c) 'push EVERY stock-on-hand order to Xero ... add the picking fee on a separate line'; business can still supersede the default later.
- **[prepaid-tag-customer-display-xero-pickfee]** ✅ IMPLEMENTED SAFE DEFAULT (pick-fee region): the NZD band applies only to explicitly recognized NZ destinations (`NZ`, `NZL`, `New Zealand`). Non-NZ or unknown destinations receive no NZ fee pending a region/currency decision.
- **[prepaid-tag-customer-display-xero-pickfee]** ✅ IMPLEMENTED DEFAULT (pick-fee band input): the lookup uses deco-inclusive goods ex-GST for the stock partition, matching the customer review and server/Xero calculation.
...
- **[prepaid-tag-customer-display-xero-pickfee]** ✅ RESOLVED (Xero prepaid-line matching): implemented the plan's primary in-memory `Set` keyed by `makeLineKey`; no `quote_items.billing_mode` snapshot migration was added.
...
- **[F2]** ✅ IMPLEMENTED DEFAULT — deferred batch send. Self-serve provisions staff with `invited_at = NULL`; `Send invites (N)` emails only pending `role='staff'` memberships in the caller's org and stamps successful sends. A future product decision may supersede this with immediate send.
